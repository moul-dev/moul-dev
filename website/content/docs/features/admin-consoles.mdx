...
description: Manage your engine via the rich browser Web Admin at /_moul_/ or the terminal-native Bubble Tea TUI (moul).
---

import { Tab, Tabs } from 'fumadocs-ui/components/tabs'
import { Callout } from 'fumadocs-ui/components/callout'

Moul provides two dedicated administrative interfaces to manage and inspect your running `mould` engine:

1. **Web Admin Console (`/_moul_/`)**: Modern responsive browser dashboard embedded directly into `mould`.
2. **Moul Terminal User Interface (`moul`)**: Fast, keyboard-driven CLI console built with Charm's Bubble Tea.
...
<Tabs items={['Interactive Mode', 'Direct Connection Flags']}>
...
| :--- | :--- | :--- |
| `-server <url>` | `mould` server URL | `-server https://api.myapp.com` |
| `-admin-key <key>` | Master administrative key | `-admin-key my-secret-key` |
| `update`, `-u`, `--update` | Self-update `moul` binary to the latest release | `moul update` |
| `-f`, `--force` | Force update even if already at latest version | `moul update -f` |
| `-v`, `--version` | Print version information and exit | `moul --version` |
