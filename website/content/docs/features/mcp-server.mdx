...
`mould` includes a native Model Context Protocol (MCP) server powered by `github.com/mark3labs/mcp-go`. This empowers AI coding assistants (Claude Desktop, Cursor, Antigravity, Windsurf) with deep inspection and operational control over your backend.

---

## Transport Modes

### 1. Stdio Mode (`mould mcp`)
...
Enabled automatically when running `mould start`. Accepts direct JSON-RPC POST requests (MCP 2025 Spec) as well as legacy SSE streams.
