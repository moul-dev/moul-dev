...
Enabled automatically when running `moul start`. Accepts direct JSON-RPC POST requests (MCP 2025 Spec) as well as legacy SSE streams. Authenticate using `Authorization: Bearer <ADMIN_SECRET_KEY>`, `X-Admin-Key: <ADMIN_SECRET_KEY>`, or URL query parameter `?adminKey=<ADMIN_SECRET_KEY>`.

---

## Assistant Configuration

<Tabs items={['Cursor & HTTP Clients', 'Claude Desktop', 'Cursor (Stdio)', 'cURL Direct JSON-RPC']}>
  <Tab value="Cursor & HTTP Clients">
Configure remote Streamable HTTP MCP in `.cursor/mcp.json` or your MCP client settings (VS Code, Windsurf, Claude Code):
...
Add to `.cursor/mcp.json` to launch a local `moul` binary process over stdio:
