...
- [`examples/custom-binary-with-mcp/`](https://github.com/moul-dev/moul-dev/tree/main/examples/custom-binary-with-mcp): Demonstrates custom binary embedding with full MCP support, custom MCP tool registration, dual HTTP auth (`Bearer` and `X-Admin-Key`), and stdio execution.
