...
`mould` can be configured via environment variables, `.env` files in the project root, or CLI flags.
...
### `mould worker list-failed` & `mould worker retry`
Inspects and manages the Dead-Letter Queue (DLQ).
```bash
# List discarded jobs in dead-letter queue
mould worker list-failed tasks_queue

# Force-retry failed jobs
mould worker retry tasks_queue [optional_job_id]
```
